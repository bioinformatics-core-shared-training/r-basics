...
Now clearly 7 is the answer but what's the `[1]` that you see before it? Well,
sometimes operations will return more than one value and R may have to print
these across several lines. Here's an example using one of the built-in datasets
that contains the lengths of the major North American rivers.
...
Now let's add several numbers together.
...
Sometimes objects are referred to as *variables*, a term commonly used in other
...
A vector is a series of values. It is the simplest data structure in R. The
`rivers` data set is an example of a vector.
...
We'll introduce functions in the next section but for now we'll note that
`c` stands for *combine* and the `c()` function combines the values it is given
...
> logical, character).
> Try different combinations of types and see what happens.

# Functions

R contains a large set of functions that do many useful operations. Let's have a
look at a simple example, the `sqrt()` function that calculates the square root
of a number.
...
The `sqrt()` function only takes a single argument. Let's try a function that
...
specify the number of digits we want it to round to using the additional
`digits` argument.
...
We can find out more information about a function by looking at its help page
by typing `?round` or `help(round)` at the command prompt.
...
Many functions take vector arguments. For example to compute the mean tumour
size we can pass our vector to the `mean()` function.
...
You may be thinking that this seems very abstract and why on earth would it ever
be useful. But actually, it is probably the most commonly used way of selecting
values of interest.
...
For example, we could obtain the sizes of tumours that are between 27.5 and
30mm.
...
We can change one or more values in a vector using the same subsetting
...
Missing values are quite common in scientific research data and R has a way of
handling these using the special value `NA`, which stands for 'not available'.
...
As an aside, a very useful function is `summary()`. As the name suggests this
produces a summary of the values it is given. It is really flexible and can take
vectors of different types, tables and other data structures.
...
This is a very interactive way of working with R but it is useful to be record
the commands you've typed when you come back to it later.

Instead we can create a script file containing our R command; this is the way
